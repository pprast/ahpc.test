...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    if (!user) return
    const myTests = db.getTestsByTeacher(user.id)
    setTests(myTests)
    if (myTests.length > 0) {
      const attempts = db.getAttemptsByTests(myTests.map(t => t.id))
      setAttemptsCount(attempts.length)
      if (attempts.length > 0) {
        setAvgScore(Math.round(attempts.reduce((sum, a) => sum + a.score, 0) / attempts.length))
      }
    }
    setLoading(false)
  }, [user?.id])
  ...
  return (
    <PageLayout>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <StatsCard title="Тестов создано" value={loading ? '—' : tests.length} icon={<BookOpen className="h-6 w-6" />} iconBg="bg-blue-50 text-blue-600" />
        <StatsCard title="Попыток сдано" value={loading ? '—' : attemptsCount} icon={<Users className="h-6 w-6" />} iconBg="bg-emerald-50 text-emerald-600" />
        <StatsCard title="Средний балл" value={loading ? '—' : `${avgScore}%`} icon={<TrendingUp className="h-6 w-6" />} iconBg="bg-amber-50 text-amber-600" />
      </div>
        ...
        <CardContent>
          {loading ? (
            <div className="space-y-4">
              {[1, 2, 3].map(i => <div key={i} className="animate-pulse bg-slate-200 h-16 rounded-lg" />)}
            </div>
          ) : tests.length === 0 ? (
            <p className="text-slate-400 text-sm text-center py-8">Нет созданных тестов</p>
          ) : (
