...
import { mockSubjects, mockTests, mockQuestions } from '../../lib/mockData'
...
export default function EditTest() {
  ...
  const existingQuestions = mockQuestions.filter(q => q.test_id === id)

  const [title, setTitle] = useState(test?.title || '')
  const [subjectId, setSubjectId] = useState(test?.subject_id || '')
  const [timeLimit, setTimeLimit] = useState(String(test?.time_limit || 30))
  const [passScore, setPassScore] = useState(String(test?.pass_score || 70))
  const [attemptsAllowed, setAttemptsAllowed] = useState(String(test?.attempts_allowed || 3))
  const [questions, setQuestions] = useState<LocalQuestion[]>(
    existingQuestions.map(q => ({
      ...
      type: q.type,
      points: q.points,
      answers: (q.answers || []).map(a => ({ localId: a.id, text: a.text, is_correct: a.is_correct })),
    }))
  )
  ...
  if (!test) return (
    <PageLayout>
      ...
      <Button className="mt-4" onClick={() => navigate('/teacher/dashboard')}>Назад</Button>
    </PageLayout>
  )
  ...
  const removeQuestion = (qId: string) => setQuestions(prev => prev.filter(q => q.localId !== qId))
  const updateQuestion = (qId: string, field: string, value: string) => setQuestions(prev => prev.map(q => q.localId === qId ? { ...q, [field]: value } : q))
  ...
  return (
    <PageLayout>
      <div className="mb-6">
        <Button variant="ghost" onClick={() => navigate('/teacher/dashboard')} className="gap-2 mb-3 -ml-2 text-slate-500">
          <ArrowLeft className="h-4 w-4" /> Назад
        </Button>
        <h1 className="text-2xl font-bold text-slate-900">Редактирование теста</h1>
        ...
        <div className="lg:col-span-1">
          <Card className="border-0 shadow-sm sticky top-20">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-1.5"><Label>Название теста</Label><Input value={title} onChange={e => setTitle(e.target.value)} /></div>
              ...
              <Button onClick={() => { alert('Тест обновлён (демо)'); navigate('/teacher/dashboard') }} className="w-full bg-[#1E40AF] hover:bg-[#1d3a9e]">Сохранить изменения</Button>
            </CardContent>
          </Card>
        </div>
